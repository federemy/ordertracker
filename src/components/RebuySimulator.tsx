...
const DEFAULT_FEE_RATE_SPOT = 0.001; // 0.10% Binance
...
/**
 * Simulador vender → recomprar
 * - Opción A: recomprar la misma cantidad → USDT sobrantes
 * - Opción B: recomprar con todo → ETH extra
 */
export function RebuySimulator({
  asset,
  currentPrice,
  defaultQty = 6,
  step = 100,
  ...
  const ladder = useMemo(() => {
    if (!valid) return [];
    const out: Array<{ target: number; leftoverA: number; ethExtraB: number }> =
      [];
    ...
    <section className="p-4 rounded-2xl border border-neutral-800 bg-neutral-900/40 grid gap-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="text-lg font-semibold">
          Si vendés <span className="font-bold">{qty}</span> {asset} ahora a{" "}
          <span className="tabular-nums">{money.format(currentPrice)}</span> y
          baja…
        </div>
