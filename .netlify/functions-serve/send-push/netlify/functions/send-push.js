...
if (PUB && PRIV) {
  import_web_push.default.setVapidDetails("mailto:you@example.com", PUB, PRIV);
}
var handler = async (event) => {
  ...
  try {
    ...
    const payload = JSON.stringify({
      title: data.title || "\u{1F514} Notificaci\xF3n",
      body: data.body || "Toc\xE1 para abrir",
      url: data.url || "/"
    });
    let targets = [];
    if (data.subscription) {
      ...
      targets = await getList(STORE, KEY) || [];
    }
    if (!targets.length) {
      return {
        statusCode: 200,
