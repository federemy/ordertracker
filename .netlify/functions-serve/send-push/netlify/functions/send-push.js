...
var handler = async (event) => {
  try {
    ...
    if (subscription?.endpoint) {
      await import_web_push.default.sendNotification(subscription, message);
      return {
        statusCode: 200,
    ...
    let subs = [];
    try {
      subs = (await getList("subs", "list") || []).filter(
        (s) => !!s?.endpoint
      );
    } catch (e) {
      return {
        statusCode: 500,
        body: "No hay suscripci\xF3n en el body y Blobs no est\xE1 configurado. Envi\xE1 { subscription } en el POST o configur\xE1 Blobs (siteID/token)."
      };
    }
    if (!subs.length) {
      return {
    ...
    await Promise.all(
      subs.map(async (s) => {
        try {
          await import_web_push.default.sendNotification(s, message);
          sent++;
        } catch {
        }
      })
